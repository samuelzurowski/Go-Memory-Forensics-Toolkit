...
import re, yara, os, logging, struct
...
class GoArtifacts(interfaces.plugins.PluginInterface):
    ...
    _32BIT = False
    yara_path = f"{os.path.dirname(os.path.realpath(__file__))}/../GoVersion.yar"
    ...
    def enum_task_struct(self, task: task_struct, proc_layer: intel.Intel, static_mode: bool) -> str:
        ...
        for offset, rule_name, name, value in proc_layer.scan(
            ...
            if name == "$magic_bytes_12" or name == "$magic_bytes_12_endian":
                break
        ...
        for task in tasks:
            ...
            if static_mode:
                for string in static_strings:
                    lib_info = string.strip().split(" ")

                    yield (0, (task.pid, comm, lib_info[0], lib_info[1], lib_info[2]))
                return
